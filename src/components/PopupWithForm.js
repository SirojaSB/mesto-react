...
function PopupWithForm({name, ariaLabel, title, button, isOpen, isClose, children}){
    return (
        <div id={`popup-${name}`} className={`popup ${isOpen && 'popup_open'}`}>
            <div className="popup__content">
                ...
                <form id={`form-${name}`} name={`form-${name}`} className={`popup__form popup__form_${name}`} noValidate>
                    {children}
